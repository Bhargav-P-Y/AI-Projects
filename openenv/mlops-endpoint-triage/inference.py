...
TASK_NAME = "mlops_triage"
BENCHMARK = "mlops_endpoint_triage"
...
async def run_eval_loop(env=None):
    ...
    action_schema = json.dumps(MLOpsAction.model_json_schema(), indent=2)
    ...
    for task_id in range(1, total_tasks + 1):
        ...
        current_task_name = f"{TASK_NAME}_task{task_id}"

        log_start(task=current_task_name, env=BENCHMARK, model=MODEL_NAME)

        messages = [
            {"role": "system", "content": f"You are an expert MLOps debugging agent. Output raw JSON only, matching this schema:\n{action_schema}"}
        ]
        ...
        try:
            while not done and step_count < MAX_STEPS:
                ...
                try:
                    ...
                    action = MLOpsAction(**action_dict)
                    ...
                    error_msg = None if obs.last_command_status == "Success" else obs.terminal_output
                    action_str = f"{action.command}('{action.filepath}')"
            ...
            results[f"task_{task_id}"] = final_score
