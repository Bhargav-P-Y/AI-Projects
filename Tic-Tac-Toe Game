...
def draw_figures(color=white):
    for row in range(board_row):
        for col in range(board_col):
            if board[row][col] == 1:
                pygame.draw.circle(screen, color, (int(col * square + square // 2), int(row * square + square // 2)),
                                   circle_rad, circle_width)
            elif board[row][col] == 2:
                pygame.draw.line(screen, color, (col * square + square // 4, row * square + square // 4),
                                 (col * square + 3 * square // 4, row * square + 3 * square // 4), cross_width)
                pygame.draw.line(screen, color, (col * square + square // 4, row * square + 3 * square // 4),
                                 (col * square + 3 * square // 4, row * square + square // 4), cross_width)
